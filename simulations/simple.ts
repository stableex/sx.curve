...
class CurveSimple {
    ...
    public D(): number {
        ...
        while (Math.abs(D - Dprev) > 1) {
            let D_P = D
            for ( const x of xp ) {
                console.log("D_P * D: ", D_P * D, ", this.n * x: ", this.n * x);
                D_P = Math.floor(D_P * D / (this.n * x))
            }
            Dprev = D
            D = Math.floor((Ann * S + D_P * this.n) * D / ((Ann - 1) * D + (this.n + 1) * D_P))

            console.log("D: ", D, "D_Prev: ", Dprev, "Delta: ", Math.abs(D - Dprev));
        }
        return D
    }
    ...
    public y(i: number, j: number, x: number): number {
        ...
        let c = D
        console.log("c * D", BigInt(c * D))
        console.log("xx[0]", xx[0])
        for (const y of xx ) {
            c = Math.floor(c * D / (y * this.n))
        }
        console.log("c2", BigInt(c))
        c = Math.floor(c * D / (this.n * Ann))
        console.log("c", BigInt(c))
...
console.log("exchange(0,1,100000)", c.exchange(0, 1, 100000));
